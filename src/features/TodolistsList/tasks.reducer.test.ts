...
test("status of specified task should be changed", () => {
  const action = tasksThunks.updateTask.fulfilled(task2, "requestId", {
    taskId: task2.taskId,
    todolistId: task2.todolistId,
    domainModel: task2.model,
  });
...
test("title of specified task should be changed", () => {
  const task3 = { taskId: "2", model: { title: "yogurt" }, todolistId: "todolistId2" };

  const action = tasksThunks.updateTask.fulfilled(task3, "requestid", {
    taskId: task3.taskId,
    todolistId: task3.todolistId,
    domainModel: task3.model,
  });
...
test("tasks should be added for todolist", () => {
  //раньше передавали просто редюсер но теперь ФУЛФИЛД! экстраредюсер просит 3 аргумента. Первый то что принимает санка
  //второй это meta данные (это нужно только для тестов, чтобы они отрабатывали, хотя эти данные мы не используем)
  // В meta данные передаем 'requestId' и параметр, который использовался в санке
  // ( в fetchTasks в качестве аргумента был todolistId, поэтому его и передаем)
